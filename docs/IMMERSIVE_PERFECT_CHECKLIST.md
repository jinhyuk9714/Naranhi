...
- [x] bot1(개발)은 매 턴 실제 파일 수정 + 테스트 + 커밋을 수행한다.
- [x] bot2(QA)는 매 턴 실패 재현 또는 회귀 테스트를 먼저 수행하고, 부족한 점을 반박한다.
- [x] 모든 완료 체크([x]) 항목은 반드시 근거(파일 경로, 테스트 명령, 결과)를 같은 커밋 또는 같은 턴 메시지에 남긴다.

## B. YouTube 핵심 기능

- [x] 팝업 `YouTube Subtitles` 토글이 watch 페이지에서 실제로 ON/OFF 응답을 주고 UI 상태가 동기화된다.
...
- [x] README 트러블슈팅에 "유튜브 자막 번역 불가" 케이스의 점검 순서가 최신화되어 있다.
- [x] TESTING 문서에 유튜브 E2E 수동 점검 시나리오와 기대 결과가 명시되어 있다.
- [x] 변경사항이 RELEASE_NOTES에 기록되어 있다.

## 종료 조건

- [x] 위 항목이 전부 [x]이고 quality gate가 통과하면 마지막 메시지에 `FLAGDOCK_DONE`을 출력한다.
