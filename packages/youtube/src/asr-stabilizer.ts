/**
 * YouTube ASR Stabilizer — sentence splitting, merging, and cue building for
 * auto-generated (ASR) captions.
 * Ported from _legacy/extension/youtubeAsrStabilizer.js
 *
 * This module contains complex, validated logic. Only types were added;
 * behaviour is preserved as-is.
 */

import { normalizeText, normalizeCaptionText } from './subtitle';

// ---------------------------------------------------------------------------
// Constants & language configs
// ---------------------------------------------------------------------------

export const DEFAULT_WORDS_REGEX =
  "etc\\.|Mr\\.|Mrs\\.|Ms\\.|Dr\\.|Prof\\.|Sr\\.|Jr\\.|U\\.S\\.|U\\.K\\.|Co\\.|Inc\\.|Ltd\\.|St\\.|p\\.a\\.|\\d+\\.";

const MANUAL_CONTINUATION_START_WORDS = new Set([
  'and', 'but', 'or', 'so', 'because', 'if', 'then', 'that',
  'which', 'who', 'when', 'while', 'to', 'for', 'of', 'in', 'on',
]);
...
interface LangConfig {
  ...
  endCompatibleConfigs?: EndCompatibleConfig[];
}

export const LANGS_CONFIG: Record<string, Partial<LangConfig>> = {
  base: {
    isSpaceLang: false,
    splitConfig: {
      minInterval: 1000,
  ...
  en: {
    ...
    splitConfig: {
      ...
      breakMiniTime: 300,
      breakWords: [
        'mhm', 'um', '>>', '- ', 'in fact', 'such as', 'or even',
        'get me', "well i'm", "i didn't", 'i know', 'i need',
        'i will', "i'll", 'i mean', 'you are', 'what does',
        'no problem', 'as we', 'if you', 'hello', 'okay', 'oh',
        'yep', 'yes', 'hey', 'hi', 'yeah', 'essentially', 'because',
        'and', 'but', 'which', 'so', 'where', 'what', 'now', 'or',
        'how', 'after',
      ],
      skipWords: ['uh'],
      minInterval: 1000,
    ...
    mergeConfig: {
      endWords: [
        'in', 'is', 'and', 'are', 'not', 'an', 'a', 'some', 'the',
        'but', 'our', 'for', 'of', 'if', 'his', 'her', 'my',
        'noticed', 'come', 'mean', 'why', 'this', 'has', 'make',
        'gpt', 'p.m', 'a.m',
      ],
      startWords: [
        'or', 'to', 'in', 'has', 'of', 'are', 'is', 'lines',
    ...
    endCompatibleConfigs: [
      { minInterval: 1000, minWordLength: 3, sentenceMinWord: 20 },
      { minInterval: 1500, minWordLength: 1, sentenceMinWord: 20 },
    ],
  },
};
...
export function calculateCueConfidence(
  text: string,
  startMs: number,
  endMs: number,
  ...
  const words =
    Number.isFinite(explicitWordCount) && explicitWordCount! > 0
      ? explicitWordCount!
...
// Pure-JS SHA-1 (no crypto dependency — runs in content scripts)
...
export function buildCueId(trackKey: string, startMs: number, endMs: number, text: string): string {
  const safeTrack = String(trackKey || 'unknown').replace(/\s+/g, '_');
  return `yt:${safeTrack}:${Math.max(0, Math.floor(startMs || 0))}:${Math.max(0, Math.floor(endMs || 0))}:${sha1Hex(text)}`;
}
...
// ---------------------------------------------------------------------------
// Language config merging
// ---------------------------------------------------------------------------

function mergeLangConfig(langCode: string): LangConfig {
  const base = deepClone(LANGS_CONFIG.base) as LangConfig;
  ...
  const merged: Record<string, unknown> = { ...(base as unknown as Record<string, unknown>), ...(deepClone(extra) as unknown as Record<string, unknown>) };
  const baseRec = base as unknown as Record<string, unknown>;
  for (const [key, value] of Object.entries(merged)) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) continue;
    if (!baseRec[key] || typeof baseRec[key] !== 'object' || Array.isArray(baseRec[key])) continue;
    if (value === baseRec[key]) continue;
    merged[key] = { ...(baseRec[key] as Record<string, unknown>), ...(value as Record<string, unknown>) };
  }
  return merged as unknown as LangConfig;
}
...
function normalizeEnglishArtifacts(tokens: Token[]): Token[] {
  const out: Token[] = [];
  let buffer: Token[] = [];
...
// ---------------------------------------------------------------------------
// Token grouping / splitting
// ---------------------------------------------------------------------------

function earlySkip(config: LangConfig, tokens: Token[]): Token | undefined {
...
function groupByTerminalPunctuation(tokens: Token[], wordRegexStr: string): Token[][] | null {
...
function tokenWordCount(group: Token[]): number {
  let text = '';
...
function breakByInterval(tokens: Token[], options: BreakOptions): Token[][] {
  ...
  function pushBreak(token: Token, forced: Token[]): void {
...
function splitAndBalance(tokens: Token[], options: BreakOptions): Token[][] {
  const firstPass = breakByInterval(tokens, options);
  const output: Token[][] = [];
  let singles: Token[] = [];

  function flushSingles(): void {
...
function mergeGroupsByBoundary(groups: Token[][], options: MergeGroupOptions): Token[][] {
...
function mergeShortTailGroups(groups: Token[][], options: EndCompatibleConfig): Token[][] {
  const out = [...groups];
  for (let i = out.length - 1; i > 0; i -= 1) {
    ...
    if (
      (Number(current[0]?.tStartMs) || 0) - (Number(previous[previous.length - 1]?.tStartMs) || 0) >
      (Number(options.minInterval) || 1000)
    )
      continue;
...
function resolveCueEnd(group: Token[], nextGroup?: Token[]): number {
...
function groupsToCues(groups: Token[][], trackKey: string, source: string): AsrCue[] {
  const cues: AsrCue[] = [];
...
export class YouTubeAsrStabilizer {
  private wordsRegex: string;
  ...
  buildCues(payload: BuildCuesPayload): AsrCue[] {
    ...
    if (symbolList?.length) {
      const splitConfig = config.splitConfig || {} as SplitConfig;
      const groups: Token[][] = [];
    ...
    const splitConfig = config.splitConfig || {} as SplitConfig;
    const mergedConfig = config.mergeConfig || {} as MergeConfig;
...
// ---------------------------------------------------------------------------
// Low-confidence ASR detection
// ---------------------------------------------------------------------------

export interface LowConfidenceOptions {
  windowMs?: number;
  minCueCount?: number;
  shortGapMs?: number;
}

export function isLowConfidenceAsrWindow(cues: AsrCue[], options?: LowConfidenceOptions): boolean {
  ...
  const overSegmented =
    list.length >= minCueCount && Number(list[list.length - 1]?.endMs || 0) - firstStart <= windowMs;
...
// ---------------------------------------------------------------------------
// Manual caption sentence merger
// ---------------------------------------------------------------------------
...
export interface SentenceMergerOptions {
  ...
  shortTailWords?: number;
}

export class ManualCaptionSentenceMerger {
  private maxGapMs: number;
  private maxChars: number;
  private maxDurationMs: number;
  private shortTailWords: number;
...
export class CueTranslationQueue {
  ...
  take(maxItems?: number): Array<{ id: string; text: string }> {
  ...
  requeue(items: Array<{ id: string; text: string }>): void {
  ...
  hasTranslated(id: string): boolean {
    const key = String(id || '').trim();
...
export class DomFallbackCommitter {
  ...
  private states = new Map<string, CommitterState>();
  private history = new Map<string, number>();

  constructor(options?: DomFallbackOptions) {
    const opts = options || {};
  ...
  dropMissingWindows(validWindowIds: Set<string> | string[]): void {
  ...
  ingest(windowKey: string, rawText: string, videoTimeMs: number, nowMs?: number): AsrCue | null {
  ...
  flush(videoTimeMs: number, nowMs?: number): AsrCue[] {
    const now = Number.isFinite(nowMs) ? nowMs! : Date.now();
    const videoMs = Number.isFinite(videoTimeMs) ? Math.max(0, Math.floor(videoTimeMs)) : 0;
    const commits: AsrCue[] = [];
  ...
  private canCommitText(text: string): boolean {
    const normalized = normalizeText(text);
    if (!normalized) return false;
  ...
  private shouldCommit(state: CommitterState, now: number): boolean {
  ...
  private commit(windowKey: string, state: CommitterState, text: string, videoMs: number, now: number): AsrCue | null {
    const normalized = normalizeText(text);
...
// ---------------------------------------------------------------------------
// selectActiveCue — find the cue active at a given time
// ---------------------------------------------------------------------------

export function selectActiveCue(cues: AsrCue[], currentMs: number): AsrCue | null {
  const list = Array.isArray(cues) ? cues : [];
  if (!list.length) return null;

  const now = Number.isFinite(currentMs) ? currentMs : 0;
  let lastPast: AsrCue | null = null;
